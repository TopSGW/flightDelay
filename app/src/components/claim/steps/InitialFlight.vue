<template>
  <div>
    ...
    <v-layout row wrap>
      ...
      <v-flex xs6 sm3>
        <v-radio :label="$t('claimform.yes')"
                 ...
                 @change="setFlightNumberIsKnown($event)"
      ...
      <v-flex xs6 sm3>
        <v-radio :label="$t('claimform.no')"
                 ...
                 @change="setFlightNumberIsUnKnown($event)"
                 v-model="flightNumberKnownValue"
    ...
    <v-flex xs12>
      <v-checkbox id="hasAConnectingFlight"
                  :label="$t('claimform.has-a-connecting-flight')"
                  v-model="connectingFlightValue"
                  trueValue="hasAConnectingFlight"
                  falseValue="noConnectingFlight"
                  @change="update('hasAConnectingFlight', $event)">
      </v-checkbox>
    </v-flex>
  </div>
</template>
...
<script>
  ...
  import {
    ...
    GET_DESTINATION_MUNICIPALITIES, GET_FLIGHT_AIRLINES, SET_DEPARTURE_AIRPORT,
    SET_DEPARTURE_MUNICIPALITY, SET_DESTINATION_AIRPORT,
    SET_DESTINATION_MUNICIPALITY, UPDATE_FLIGHT,
  ...
  export default {
    ...
    filters: {
      dateFormat: (value) => {
        ...
        console.log({ method: 'dateFormat', value, formattedValue });
    ...
    computed: {
      ...mapState({
        ...
        delay: state => state.claimForm.flights.all.getInitialFlight().delay,
        flightNumberIsKnown: state => state.claimForm.flights.all.getInitialFlight().flightNumberIsKnown,
        flightDate: state => state.claimForm.flights.all.getInitialFlight().flightDate,
        flightNumber: state => state.claimForm.flights.all.getInitialFlight().flightNumber,
        departureMunicipality: state => state.claimForm.flights.all.getInitialFlight().departureMunicipality,
        departureAirports: state => state.claimForm.flights.all.getInitialFlight().departureAirports,
        departureAirport: state => state.claimForm.flights.all.getInitialFlight().departureAirport,
        destinationMunicipality: state => state.claimForm.flights.all.getInitialFlight().destinationMunicipality,
        destinationAirports: state => state.claimForm.flights.all.getInitialFlight().destinationAirports,
        destinationAirport: state => state.claimForm.flights.all.getInitialFlight().destinationAirport,
        airline: state => state.claimForm.flights.all.getInitialFlight().airline,
        airlines: state => state.claimForm.flights.all.getInitialFlight().airlines,
        hasAConnectingFlight: state => state.claimForm.flights.all.getInitialFlight().hasAConnectingFlight,
    ...
    updated() {
      ...
      this.flightNumberKnownValue = this.flightNumberIsKnown === true ? 'flightNumberIsKnown' : 'flightNumberIsUnKnown';
    ...
    methods: {
      ...
      setFlightNumberIsKnown(value) {
        this.update('flightNumberIsKnown', value);
        this.resetDirty('flightNumberUnKnown');
      },
      setFlightNumberIsUnKnown(value) {
        this.update('flightNumberIsUnKnown', value);
        this.resetDirty('flightNumberKnown');
      },
      // eslint-disable-next-line
      update(field, e) {
        let value = null;
